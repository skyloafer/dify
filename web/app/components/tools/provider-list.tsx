...
const ProviderList = () => {
  ...
  const currentProvider = useMemo<Collection | undefined>(() => {
    return filteredCollectionList.find(
      (collection) => collection.id === currentProviderId,
    );
  ...
  const [isMarketplaceArrowVisible, setIsMarketplaceArrowVisible] =
    useState(true);
    ...
    <>
      <div className="relative flex h-0 shrink-0 grow overflow-hidden">
        ...
        >
          <div
            className={cn(
              "sticky top-0 z-10  gap-y-2 bg-background-body px-12 pb-2 pt-4 leading-[56px]",
          ...
          >
            ...
            <div className="flex flex-wrap items-center justify-between">
              <TabSliderNew
                ...
                onChange={(state) => {
                  setActiveTab(state);
                  if (state !== activeTab) setCurrentProviderId(undefined);
                }}
                options={options}
              />
          ...
          {activeTab !== "mcp" && (
            <div
              className={cn(
                "relative grid shrink-0 grid-cols-1 content-start gap-4 px-12 pb-4 pt-2 sm:grid-cols-3",
          ...
          {!filteredCollectionList.length && activeTab === "builtin" && (
            <Empty
              ...
              className="h-[224px] shrink-0 px-12"
            />
          )}
          ...
          {activeTab === "mcp" && <MCPList searchText={keywords} />}
        </div>
      </div>
