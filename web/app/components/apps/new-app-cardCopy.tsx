...
const CreateAppTemplateDialog = dynamic(
  () => import("@/app/components/app/create-app-dialog"),
  {
    ssr: false,
  },
);
const CreateFromDSLModal = dynamic(
  () => import("@/app/components/app/create-from-dsl-modal"),
  {
    ssr: false,
  },
);
...
const CreateAppCard = ({ ref, className, onSuccess }: CreateAppCardProps) => {
  ...
  const [showCreateFromDSLModal, setShowCreateFromDSLModal] =
    useState(!!dslUrl);
  ...
  return (
    <div
      ref={ref}
      className={cn(
        "relative col-span-1 inline-flex h-[160px] flex-col justify-between rounded-xl border-[0.5px] border-components-card-border bg-components-card-bg",
        className,
      )}
    >
