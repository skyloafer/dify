...
const EditAppModal = dynamic(
  () => import("@/app/components/explore/create-app-modal"),
  {
    ssr: false,
  },
);
const DuplicateAppModal = dynamic(
  () => import("@/app/components/app/duplicate-modal"),
  {
    ssr: false,
  },
);
const SwitchAppModal = dynamic(
  () => import("@/app/components/app/switch-app-modal"),
  {
    ssr: false,
  },
);
...
const AccessControl = dynamic(
  () => import("@/app/components/app/app-access-control"),
  {
    ssr: false,
  },
);
...
const AppCard = ({ app, onRefresh }: AppCardProps) => {
  ...
  const Operations = (props: HtmlContentProps) => {
    const { data: userCanAccessApp, isLoading: isGettingUserCanAccessApp } =
      useGetUserCanAccessApp({
        appId: app?.id,
        enabled: !!props?.open && systemFeatures.webapp_auth.enabled,
      });
    ...
    const onClickAccessControl = async (
    ...
    const onClickInstalledApp = async (
      ...
      try {
        ...
        if (installed_apps?.length > 0)
          window.open(
            `${basePath}/explore/installed/${installed_apps[0].id}`,
            "_blank",
          );
    ...
    return (
      <div
        className="relative flex w-full flex-col py-1"
        ...
        <div className="flex h-[66px] shrink-0 grow-0 items-center gap-3 px-[14px] pb-3 pt-[14px]">
          ...
          <div className="flex h-5 w-5 shrink-0 items-center justify-center">
            {app.access_mode === AccessMode.PUBLIC && (
              <Tooltip
                asChild={false}
                popupContent={t("app.accessItemsDescription.anyone")}
              >
            ...
            {app.access_mode === AccessMode.SPECIFIC_GROUPS_MEMBERS && (
              <Tooltip
                asChild={false}
                popupContent={t("app.accessItemsDescription.specific")}
              >
            ...
            {app.access_mode === AccessMode.ORGANIZATION && (
              <Tooltip
                asChild={false}
                popupContent={t("app.accessItemsDescription.organization")}
              >
            ...
            {app.access_mode === AccessMode.EXTERNAL_MEMBERS && (
              <Tooltip
                asChild={false}
                popupContent={t("app.accessItemsDescription.external")}
              >
              ...
              <div className="!hidden shrink-0 group-hover:!flex">
                <CustomPopover
                  ...
                  btnClassName={(open) =>
                    cn(
                      open
                        ? "!bg-state-base-hover !shadow-none"
                        : "!bg-transparent",
