...
const TagSelector: FC<TagSelectorProps> = ({
  ...
  const triggerContent = useMemo(() => {
    if (selectedTags?.length)
      return selectedTags
        .filter((selectedTag) =>
          tagList.find((tag) => tag.id === selectedTag.id),
        )
        .map((tag) => tag.name)
        .join(", ");
    return "";
      ...
      >
        ...
        <div
          className={cn(
            ...
            hasNoTags
              ? "italic text-components-input-text-placeholder"
              : "font-medium text-components-input-text-placeholder",
          )}
        >
